...
function playSoundForGameEvent(event: GameState['events'][number]) {
  ...
  if (event.type === 'power') {
    ...
    else if (message.includes('tire') || message.includes('projectile')) {
      playSound('shoot', 0.85);
    } else if (message.includes('invisible')) {
      playSound('invisibility', 0.75);
    } else if (message.includes('freeze') || message.includes('gèle')) {
      playSound('freeze', 0.8);
    } else if (message.includes('mur')) {
      playSound('wall', 0.75);
    }
  }
}
...
export const GameCanvas = ({
  ...
  useEffect(() => {
    ...
    let renderedPlayers: GameState['players'] = {};
    ...
    const updateRenderedPlayers = () => {
      const serverPlayers = serverState.players;

      Object.entries(serverPlayers).forEach(([id, serverPlayer]) => {
        const currentRenderedPlayer = renderedPlayers[id];

        if (!currentRenderedPlayer) {
          renderedPlayers[id] = { ...serverPlayer };
          return;
        }

        renderedPlayers[id] = {
          ...serverPlayer,
          x: lerp(currentRenderedPlayer.x, serverPlayer.x, 0.22),
          y: lerp(currentRenderedPlayer.y, serverPlayer.y, 0.22),
        };
      });

      Object.keys(renderedPlayers).forEach((id) => {
        if (!serverPlayers[id]) {
          delete renderedPlayers[id];
        }
      });
    };
    ...
    const handleGameState = (state: GameState) => {
      ...
      if (myPlayer?.role) {
        setMobileRole((currentRole) =>
          currentRole === myPlayer.role ? currentRole : myPlayer.role,
        );
      }
      ...
      if (state.status === 'finished') {
        ...
        if (!gameOverNotified) {
          ...
          const currentPlayer = state.players[mySocketId];

          playSound(
            currentPlayer?.role === state.winnerTeam ? 'victory' : 'defeat',
            0.9,
          );
    ...
    const handleJoinRejected = (data: { reason?: string }) => {
      ...
      onJoinRejectedRef.current?.(
        data.reason ?? 'La partie est déjà complète.',
      );
    };
    ...
    const handleGameOver = (payload: GameOverPayload) => {
      ...
      if (!gameOverNotified) {
        ...
        playSound(
          myPlayer?.role === payload.winnerTeam ? 'victory' : 'defeat',
          0.9,
        );
    ...
    function screen() {
      ...
      const renderedMyPlayer = renderedPlayers[mySocketId];
      const serverMyPlayer = serverState.players[mySocketId];
      ...
      drawRadarArrow(ctx, serverMyPlayer, camera);
      drawPlayers(ctx, renderedPlayers, camera, mySocketId);
      drawParticles(ctx, particles, camera);

      drawMatchHud(ctx, serverState.players, {
        width: window.innerWidth,
        height: window.innerHeight,
      } as HTMLCanvasElement);

      drawEventFeed(ctx, serverState.events, {
        width: window.innerWidth,
        height: window.innerHeight,
      } as HTMLCanvasElement);

      if (serverMyPlayer) {
        drawPowerHud(ctx, serverMyPlayer, {
          width: window.innerWidth,
          height: window.innerHeight,
        } as HTMLCanvasElement);
      }
      ...
      if (serverState.status === 'waiting') {
        ...
        ctx.fillText(
          'Matchmaking : 1 chasseur rouge contre 1 fuyard bleu',
          window.innerWidth / 2,
      ...
      if (game.isOver) {
        if (renderedMyPlayer && !game.explosion) {
          createExplosion(
            renderedMyPlayer.x,
            renderedMyPlayer.y,
            particles,
            renderedMyPlayer.color,
          );
        ...
        if (particles.length === 0) {
          endScreen(
            ...
            result || game.result || 'Fin de partie',
          );
        }
      }
  ...
  const setMoveKey = (
    direction: keyof typeof MOBILE_CONTROL_KEYS,
    active: boolean,
  ) => {
      ...
      <div className="mobile-controls" onPointerLeave={stopAllMobileMovement}>
        <div className="mobile-dpad" aria-label="Contrôles de déplacement mobile">
          <button
            ...
            onPointerDown={() => setMoveKey('up', true)}
            onPointerUp={() => setMoveKey('up', false)}
            onPointerCancel={() => setMoveKey('up', false)}
          >
            Z
          </button>
          ...
          <button
            ...
            onPointerDown={() => setMoveKey('down', true)}
            onPointerUp={() => setMoveKey('down', false)}
            onPointerCancel={() => setMoveKey('down', false)}
          >
            S
          </button>
        ...
        <div className="mobile-powers" aria-label="Pouvoirs mobile">
          {getMobilePowerLabels(mobileRole).map((power) => (
            <button
              key={power.power}
              type="button"
              onClick={() => useMobilePower(power.power)}
            >
